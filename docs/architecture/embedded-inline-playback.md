...
Chromium releases the MediaSource/SourceBuffer resources of a detached player asynchronously, so rapid channel zapping across MPEG-TS streams can exhaust the SourceBuffer quota (`QuotaExceededError`: "reached the limit of SourceBuffer objects") even though every player instance is destroyed correctly. The HTML5 player owns the mpegts.js lifecycle through `MpegtsPlayerController` (`libs/ui/playback/src/lib/html-video-player/mpegts-player-controller.ts`), which detects that quota error, tears the player down, resets the media element, and retries with a fresh MediaSource after a short delay (max 2 retries). Only exhausted retries surface a diagnostic, classified as `unknown-playback-error` rather than `unsupported-container` — the quota is a transient resource limit, not a container capability problem.
